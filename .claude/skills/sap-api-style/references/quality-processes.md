<!--
Modified 2026-08-29 by Systems LAB 5, as GPL-3.0 section 5 requires this to
be stated.

One example Stripe key was replaced with sk_live_EXAMPLE_NOT_A_REAL_KEY.
It appeared in a sample captioned as a security risk — the point being made
was to NOT hardcode keys — but it carried the sk_live_ prefix and a
24-character body, which is the shape of a real one. GitHub's push
protection blocked it, correctly: a scanner cannot tell a teaching example
from a leak, and neither can anyone reading the file.

Nothing else in this file is changed. The lesson it teaches is unaffected.
-->
